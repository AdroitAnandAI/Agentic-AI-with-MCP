...
def cos(a: int) -> float:
    ...
    return float(math.cos(a*math.pi/180))
...
def tan(a: int) -> float:
    ...
    return float(math.tan(a*math.pi/180))
...
@mcp.tool()
def show_reasoning(steps: Union[List[str], Dict[str, List[str]]]) -> dict:
    """Show the step-by-step reasoning process"""
    print("+++++++++++++++++++ show_reasoning called with steps = " + str(steps))
...
def calculate(expression: str) -> dict:
    ...
    print("+++++++++++++++++++ calculate called with expression = " + str(expression))
...
@mcp.tool()
def check_consistency(steps: List[Dict[str, Any]]) -> TextContent:
...
@mcp.tool()
def fallback_reasoning(stepdesc: str) -> TextContent:
    """Fallbacks for Errors or Uncertainty"""
    console.print("[blue]FUNCTION CALL:[/blue] calculate()")
    console.print(f"[blue]Expression:[/blue] {stepdesc}")
    try:
        result = eval(stepdesc)
        console.print(f"[red]Result:[/red] {stepdesc}")
        return TextContent(
            type="text",
            text=str(stepdesc)
        )
...
async def add_text_in_paint(text: str) -> dict:
    """To write the result in Paint inside the rectangle"""
    global paint_app
    try:
        if not paint_app:
            return {
...
async def open_paint() -> dict:
    """To open Microsoft Paint"""
    global paint_app
    try:
        print('Inside open_paint')
        ...
        return {
            "content": [
                TextContent(
                    type="text",
                    text="Microsoft Paint has been already opened successfully"
                )
            ]
        }
    except Exception as e:
...
def debug_error(error: str) -> list[base.Message]:
    ...
    return [base.Message(content=error)]
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Error starting MCP server: {str(e)}", flush=True)
        import traceback
