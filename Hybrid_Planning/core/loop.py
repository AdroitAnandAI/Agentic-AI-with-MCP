...
from modules.tools import summarize_tools, get_similar_memory_queries, prioritize_search_tools
import re
...
class AgentLoop:
    ...
    async def run(self):
        ...
        similar_queries = get_similar_memory_queries(self.context.user_input, self.context.memory.get_session_items())
        if similar_queries:
            # Use the most recent successful answer
            for item in reversed(similar_queries):
                if item.success and item.text:
                    log("strategy", f"✅ Found similar query in memory with successful answer")
                    # return f"FINAL_ANSWER: {item['final_answer']}"
                    return {"status": "done", "result": item.tool_result.get('result')}
        ...
        for step in range(max_steps):
            ...
            while lifelines_left >= 0:
                ...
                # Prioritize tools based on query characteristics
                if selected_tools:
                    selected_tools = prioritize_search_tools(
                        tools=selected_tools,
                        query=user_input_override or self.context.user_input
                    )
                    log("loop", f"🔍 Prioritized tools: {[tool.name for tool in selected_tools]}")
