...
def check_ollama_connection():
    ...
    try:
        ...
        print("Please make sure Ollama is running at http://localhost:11434")
        return False
...
def search_stored_documents(input: SearchDocumentsInput) -> list[str]:
    ...
    print("CALLED: search_stored_documents", file=sys.stderr)
    ensure_faiss_ready()
    query = input.query
...
def ensure_faiss_ready():
    ...
    try:
        ...
        mcp_log("ERROR", f"Failed to initialize FAISS index: {str(e)}")
        # Create empty index if initialization fails
        if not index_path.exists():
            dim = 384  # Default dimension for nomic-embed-text
            index = faiss.IndexFlatL2(dim)
            faiss.write_index(index, str(index_path))
            meta_path.write_text("[]")
            mcp_log("INFO", "Created empty FAISS index as fallback")


if __name__ == "__main__":
    print("mcp_server_2.py starting", file=sys.stderr)
    
    # Check Ollama connection first
    if not check_ollama_connection():
        print("Failed to connect to Ollama. Server will start with limited functionality.", file=sys.stderr)
        print("Document processing and embeddings will be disabled.", file=sys.stderr)
        print("Please start Ollama and restart the server for full functionality.", file=sys.stderr)
